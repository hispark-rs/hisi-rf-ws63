...
- Re-export the SLE address and connected-state ABI values needed by the
  bounded coexistence controller, so facade consumers no longer depend on
  `ws63-radio-sys` directly.
...
- Re-export the shared coexistence `Resources` capability for the WPA2
  SoftAP plus BLE target configuration, matching the already verified SLE
  composition and allowing the standalone AP fixture to final-link.
