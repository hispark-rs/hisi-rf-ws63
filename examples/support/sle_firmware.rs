...
hisi_rf_ws63::declare_sle_s1_storage!(static SLE_STORAGE);

pub fn run(role: fn(&mut hisi_rf_ws63::SleS1Controller) -> !) -> ! {
    ...
    let storage = SLE_STORAGE.install().expect("install SLE storage");
    ...
    let _runtime = hisi_rtos::start_with_port(
        hisi_rtos::PortedConfig {
            minimum_stack_size: NonZeroUsize::new(hisi_rf_ws63::SLE_S1_MINIMUM_TASK_STACK_BYTES)
                .unwrap(),
    ...
    .expect("start SLE RTOS");

    unsafe { interrupt::enable_global() };
    hisi_rtos::request_reschedule();
    let resources = hisi_rf_ws63::SleS1Resources::new(efuse, p.KM, p.SPACC, p.TRNG);
    match hisi_rf_ws63::init_sle_s1(resources, storage) {
        Ok(mut controller) => {
            log(b"RFDBG_SLE_S1_INIT_OK\r\n");
            role(&mut controller)
        }
        Err(error) => {
            log(b"RFDBG_SLE_S1_INIT_ERR code=0x");
...
fn error_code(error: hisi_rf_ws63::SleS1InitError) -> u32 {
    use hisi_rf_ws63::SleS1InitError;
    match error {
        SleS1InitError::StorageAlreadyInstalled => 1,
        SleS1InitError::InsufficientArena { .. } => 2,
        SleS1InitError::AllocatorInstall => 3,
        SleS1InitError::TaskPlan => 4,
        SleS1InitError::TaskAdmission => 5,
        SleS1InitError::SchedulerLock => 6,
        SleS1InitError::TaskSpawn { index } => 0x100 + index as u32,
        SleS1InitError::SchedulerUnlock => 7,
        SleS1InitError::Crypto => 8,
