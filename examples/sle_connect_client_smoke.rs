//! Credential-free WS63 SLE S2 seek/connect/disconnect client smoke.
            ...
            match event {
                ...
                hisi_rf_ws63::SleS1Event::SeekDisabled { status: 0 } if connect_after_stop => {
                    if controller.connect(&SERVER_ADDRESS).is_err() {
                        fail(b"RFDBG_SLE_S2_CLIENT_CONNECT_ERR\r\n");
                    }
                ...
                hisi_rf_ws63::SleS1Event::ConnectionStateChanged {
                    ...
                    sle_firmware::log(b"RFDBG_SLE_S2_CLIENT_CONNECTED\r\n");
                    disconnect_delay = Some(10u8);
                }
                hisi_rf_ws63::SleS1Event::ConnectionStateChanged {
                    connection_state: CONNECTION_STATE_DISCONNECTED,
                    ..
                } => {
                    sle_firmware::log(b"RFDBG_SLE_S2_CONNECT_DISCONNECT_OK\r\n");
                }
                _ => {}
            }
        }
        if let Some(remaining) = disconnect_delay {
            if remaining == 0 {
                if controller.disconnect(&SERVER_ADDRESS).is_err() {
                    fail(b"RFDBG_SLE_S2_CLIENT_DISCONNECT_ERR\r\n");
                }
                disconnect_delay = None;
            } else {
                disconnect_delay = Some(remaining - 1);
            }
        }
        if controller.dropped_events() != 0 {
            fail(b"RFDBG_SLE_S2_CLIENT_EVENT_DROP\r\n");
        }
        sle_firmware::sleep();
    }
}
